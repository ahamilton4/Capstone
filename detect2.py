...
def detect(save_txt=False, save_img=False):
    ...
    t0 = time.time()
    framecount = 0
    with open('detected.csv',mode='w') as csv_file:
        fieldnames = ['filename','width','height','vehicle','xmin','ymin','xmax','ymax']
                ...
                if det is not None and len(det):
                    ...
                    for *xyxy, conf, cls in det:
                        ...
                        if save_img or view_img:  # Add bbox to image
                            label = 'car'
                        writer.writerow({'filename': path, 'width': width, 'height': height, 'vehicle': 'vehcile', 'xmin': xmin,'ymin': ymin, 'xmax': xmax, 'ymax': ymax})
